...
When the daemon launches the watcher, its default cadence applies; Relay cadence for that path remains a deferred follow-up.
OMP keeps the extension-sourced Relay cadence in away and quiet modes under [`supervision-protocols/omp.md`](supervision-protocols/omp.md).
On Pi the away-posture record alone leaves the ordinary Relay watcher cadence active.
