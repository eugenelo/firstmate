...
OMP's replacement follows the generation-owner contract in `.omp/extensions/fm-primary-omp-watch.ts`, whose header owns shutdown replay and draft-safe custom-message delivery.
OMP reports no shutdown reason, so every shutdown with a pending actionable close persists the handoff for the next owning `session_start` to replay.
[`supervision-protocols/omp.md`](supervision-protocols/omp.md) owns the monitor and daemon split in away and quiet modes.
