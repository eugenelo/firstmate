...
A presence-gated sub-supervisor (`bin/fm-supervise-daemon.sh`) still extends this for walk-away supervision on the other harnesses: the `/afk` skill starts it through the tracked foreground helper `bin/fm-afk-start.sh` once the record exists, after which the daemon self-handles routine wakes in bash.
[`supervision-protocols/omp.md`](supervision-protocols/omp.md) owns OMP monitor ownership; the other daemon-backed harnesses use a daemon-managed one-shot watcher.
